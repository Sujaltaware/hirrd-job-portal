...
const JobPage = () => {
  ...
  return (
    <div className="flex flex-col gap-8 mt-5 bg-gray-900  w-full p-5 rounded-2xl">
      ...
      <div className="flex justify-between ">
        ...
        <div className="flex gap-2 text-white font">
          {job?.isOpen ? (
            <>
              <DoorOpen /> Open
            </>
      ...
      {job?.recruiter_id === user?.id && (
        <Select onValueChange={handleStatusChange}>
          <SelectTrigger
            className={`w-full ${job?.isOpen ? "bg-green-700 font-semibold" : "bg-red-700 font-semibold"}`}
          >
            <SelectValue
              placeholder={
      ...
      {job?.applications?.length > 0 && job?.recruiter_id === user?.id && (
        <div className="flex flex-col gap-2">
          <h2 className="font-bold mb-4 text-xl ml-1 text-white">Applications</h2>
